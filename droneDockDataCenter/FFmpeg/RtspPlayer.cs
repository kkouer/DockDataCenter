...
namespace droneDockDataCenter.FFMpeg
{
    public unsafe class RtspPlayer
    {
        ...
        public unsafe void Start(/*ShowBitmapDelegate show,*/ string url)
        {
            ...
            Debug.WriteLine(@"Runnung in {0}-bit mode.", Environment.Is64BitProcess ? @"64" : @"32");
            //FFmpegDLL目录查找和设置
            ...
            Debug.WriteLine($"codec name: {ffmpeg.avcodec_get_name(codecContext.codec_id)}");
            // 获取图像的宽、高及像素格式
            ...
            while (CanRun)
            {
                try
                {
                    ...
                    //Debug.WriteLine($@"frame: {frameNumber}");
                    // YUV->RGB
